...
#testing
new_djdata = gen_wrong("../../stock_data/djia_2012.csv")
dj_df.compare(new_djdata)
new_djdata.to_csv('new_djdata.csv', encoding= 'utf-8', index=False)
...
nasdaq_df = pd.read_csv("../../stock_data/nasdaq_all.csv") 
# nas_df = pd.read_csv("stock_data/nasdaq_all.csv") 
# rus_df = pd.read_csv("stock_data/russel2000_all.csv") 
...
def gen_wrong(sample):
    ...
    df_copy = df.copy() #make copy of dataframe to apply change
    avg_length_crash = 342 #average length of stock market crash
    n = random.randint(5, 30) #num intervals
    noise_levels = [10, 20, 30, 40, 50] #for noise
    for i in range(n):
        num_rows = random.randint(1, avg_length_crash) #generate random number of rows for interval
        start_idx = random.randint(0, len(df) - num_rows) #start idx for intervall
        ...
        for j in range(len(noise_levels)): #apply noise to copy of df
            noise = np.random.normal(0, noise_levels[j], num_rows) #generate random amt of noise
            print(noise)
            interval.iloc[:, 6] += noise #add noise
            df_copy.iloc[start_idx:end_idx] = interval #add interval to df copy
    return df_copy


new_nasdaqdata = gen_wrong("../../stock_data/nasdaq_all.csv")
nasdaq_df.compare(new_nasdaqdata)
new_nasdaqdata.to_csv('new_nasdaq.csv', encoding= 'utf-8', index=False)
...
russel_df = pd.read_csv("../../stock_data/russel2000_all.csv") 
# nas_df = pd.read_csv("stock_data/nasdaq_all.csv") 
# rus_df = pd.read_csv("stock_data/russel2000_all.csv") 
...
new_russeldata = gen_wrong("../../stock_data/russel2000_all.csv")
russel_df.compare(new_russeldata)
new_russeldata.to_csv('new_russel.csv', encoding= 'utf-8', index=False)
